...
import { IColumn, IBoardMember } from '../models/Board.js';
...
export function registerSocketHandlers(io: Server) {
  io.on('connection', (socket: Socket) => {
    ...
    // --- Room Join/Leave ---
    socket.on(SOCKET_EVENTS.JOIN_BOARD, async (payload: JoinBoardPayload, ack?: (response: AckSuccess | AckError) => void) => {
      try {
    ...
    socket.on(SOCKET_EVENTS.LEAVE_BOARD, async (payload: LeaveBoardPayload, ack?: (response: AckSuccess | AckError) => void) => {
      try {
        const { boardId } = payload;
    ...
    // --- Task Events ---
    socket.on(SOCKET_EVENTS.CREATE_TASK, async (payload: CreateTaskPayload, ack?: (response: AckSuccess | AckError) => void) => {
      try {
        ...
        const taskId = payload.task.id || new mongoose.Types.ObjectId().toString();
        const task = { 
          ...payload.task, 
          id: taskId, 
          createdBy: user.id, 
        ...
        io.to(payload.boardId).emit(SOCKET_EVENTS.TASK_CREATED, { 
    ...
    socket.on(SOCKET_EVENTS.UPDATE_TASK, async (payload: UpdateTaskPayload, ack?: (response: AckSuccess | AckError) => void) => {
      try {
        if (!await hasBoardPermission(payload.boardId, user.id, 'edit')) throw new Error('No permission');
        const board = await Board.findOne({ id: payload.boardId });
        if (!board) throw new Error('Board not found');
        ...
        io.to(payload.boardId).emit(SOCKET_EVENTS.TASK_UPDATED, { 
          boardId: payload.boardId, 
    ...
    socket.on(SOCKET_EVENTS.DELETE_TASK, async (payload: DeleteTaskPayload, ack?: (response: AckSuccess | AckError) => void) => {
      try {
        if (!await hasBoardPermission(payload.boardId, user.id, 'edit')) throw new Error('No permission');
        const board = await Board.findOne({ id: payload.boardId });
        if (!board) throw new Error('Board not found');
        ...
        io.to(payload.boardId).emit(SOCKET_EVENTS.TASK_DELETED, { 
          boardId: payload.boardId, 
          taskId: payload.taskId, 
          sourceClientId: socket.id 
        });
    ...
    socket.on(SOCKET_EVENTS.MOVE_TASK, async (payload: MoveTaskPayload, ack?: (response: AckSuccess | AckError) => void) => {
      try {
        if (!await hasBoardPermission(payload.boardId, user.id, 'edit')) throw new Error('No permission');
        const board = await Board.findOne({ id: payload.boardId });
        if (!board) throw new Error('Board not found');
    ...
    socket.on(SOCKET_EVENTS.REORDER_TASKS, async (payload: ReorderTasksPayload, ack?: (response: AckSuccess | AckError) => void) => {
      try {
        if (!await hasBoardPermission(payload.boardId, user.id, 'edit')) throw new Error('No permission');
        const board = await Board.findOne({ id: payload.boardId });
        if (!board) throw new Error('Board not found');
    ...
    // --- Column Events ---
    socket.on(SOCKET_EVENTS.CREATE_COLUMN, async (payload: CreateColumnPayload, ack?: (response: AckSuccess | AckError) => void) => {
      try {
        if (!await hasBoardPermission(payload.boardId, user.id, 'edit')) throw new Error('No permission');
        const board = await Board.findOne({ id: payload.boardId });
        if (!board) throw new Error('Board not found');
        ...
        io.to(payload.boardId).emit(SOCKET_EVENTS.COLUMN_CREATED, { 
          boardId: payload.boardId, 
          column, 
          sourceClientId: socket.id 
        });
    ...
    socket.on(SOCKET_EVENTS.UPDATE_COLUMN, async (payload: UpdateColumnPayload, ack?: (response: AckSuccess | AckError) => void) => {
      try {
        if (!await hasBoardPermission(payload.boardId, user.id, 'edit')) throw new Error('No permission');
        const board = await Board.findOne({ id: payload.boardId });
        if (!board) throw new Error('Board not found');
        ...
        io.to(payload.boardId).emit(SOCKET_EVENTS.COLUMN_UPDATED, { 
          boardId: payload.boardId, 
          columnId: payload.columnId, 
          updates: payload.updates, 
          sourceClientId: socket.id 
    ...
    socket.on(SOCKET_EVENTS.DELETE_COLUMN, async (payload: DeleteColumnPayload, ack?: (response: AckSuccess | AckError) => void) => {
      try {
        if (!await hasBoardPermission(payload.boardId, user.id, 'edit')) throw new Error('No permission');
        const board = await Board.findOne({ id: payload.boardId });
        if (!board) throw new Error('Board not found');
        ...
        io.to(payload.boardId).emit(SOCKET_EVENTS.COLUMN_DELETED, { 
          boardId: payload.boardId, 
          columnId: payload.columnId, 
          sourceClientId: socket.id 
        });
    ...
    socket.on(SOCKET_EVENTS.REORDER_COLUMNS, async (payload: ReorderColumnsPayload, ack?: (response: AckSuccess | AckError) => void) => {
      try {
        if (!await hasBoardPermission(payload.boardId, user.id, 'edit')) throw new Error('No permission');
        const board = await Board.findOne({ id: payload.boardId });
        if (!board) throw new Error('Board not found');
        
        board.columns = payload.columnIds
          .map((id: string) => board.columns.find((c: any) => c.id === id))
    ...
    socket.on(SOCKET_EVENTS.ADD_MEMBER, async (payload: AddMemberPayload, ack?: (response: AckSuccess | AckError) => void) => {
      try {
        ...
        board.members.push(newMember as IBoardMember);
        await board.save();
    ...
    socket.on(SOCKET_EVENTS.REMOVE_MEMBER, async (payload: RemoveMemberPayload, ack?: (response: AckSuccess | AckError) => void) => {
      try {
    ...
    socket.on(SOCKET_EVENTS.CHANGE_ROLE, async (payload: ChangeRolePayload, ack?: (response: AckSuccess | AckError) => void) => {
      try {
        if (!await hasBoardPermission(payload.boardId, user.id, 'manage')) throw new Error('No permission');
        const board = await Board.findOne({ id: payload.boardId });
        if (!board) throw new Error('Board not found');
        ...
        if (!validRoles.includes(payload.role as any)) throw new Error('Invalid role');
        member.role = payload.role as typeof validRoles[number];
    ...
    socket.on(SOCKET_EVENTS.UPDATE_PERMISSIONS, async (payload: UpdatePermissionsPayload, ack?: (response: AckSuccess | AckError) => void) => {
      try {
        if (!await hasBoardPermission(payload.boardId, user.id, 'manage')) throw new Error('No permission');
        const board = await Board.findOne({ id: payload.boardId });
        if (!board) throw new Error('Board not found');
    ...
    // --- Board Events ---
    socket.on(SOCKET_EVENTS.UPDATE_BOARD, async (payload: UpdateBoardPayload, ack?: (response: AckSuccess | AckError) => void) => {
      try {
        if (!await hasBoardPermission(payload.boardId, user.id, 'edit')) throw new Error('No permission');
        const board = await Board.findOne({ id: payload.boardId });
        if (!board) throw new Error('Board not found');
    ...
    socket.on(SOCKET_EVENTS.DELETE_BOARD, async (payload: DeleteBoardPayload, ack?: (response: AckSuccess | AckError) => void) => {
      try {
