...
const Column: React.FC<ColumnProps> = ({ column, boardId, children }) => {
  const { updateColumn, deleteColumn, createTask, users, fetchUsers } = useBoardStore();
  ...
  const handleAddTask = async () => {
    ...
    // Validate assignee - only use if it's a valid user ID
    let finalAssignee = currentUserId; // Default to current user
        ...
        <div className="space-y-4">
          ...
          <div className="flex gap-2">
            <Dropdown
              ...
              onChange={val => setNewTaskPriority(val as 'low' | 'medium' | 'high')}
              placeholder="Priority"
            />
            <Dropdown
              options={users.length > 0 ? users.map(u => ({ value: u.id, label: u.name })) : [{ value: '', label: 'No users available' }]}
          ...
          <div className="flex justify-end gap-2">
            ...
            <Button variant="primary" onClick={handleAddTask} loading={isLoading}>Add Task</Button>
          </div>
        </div>
      </Modal>
    </motion.div>
